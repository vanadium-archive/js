...
func (i *invokeMethWCavIdImpl) Invoke(servCtx ipc.ServerContext, name string, cavDesc security.CaveatDescriptor, cavParam *vdl.Value) error {
	ctx := servCtx.Context()
	...
	other, _ := servCtx.RemoteBlessings().ForContext(servCtx)
