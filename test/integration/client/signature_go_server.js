...
describe('client/signature_go_server.js: ' +
  ...
  it('Should use a valid cache entry', function(done) {
    ...
    veyron.init(TestHelper.veyronConfig, function(err, rt) {
      ...
      rt._getProxyConnection().bindCache[absoluteVeyronName] = {
        fetched: new Date(),
        signature: {
          bar: 'foo'
        }
      ...
      rt._getProxyConnection().getServiceSignature(absoluteVeyronName)
      .then(function(sig) {
  ...
  it('Should store a valid cache entry', function(done) {
    ...
    veyron.init(TestHelper.veyronConfig, function(err, rt) {
      ...
      rt._getProxyConnection().getServiceSignature(absoluteVeyronName)
      .then(function() {
        var entry = rt._getProxyConnection().bindCache[absoluteVeyronName];
